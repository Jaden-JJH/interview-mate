// 인앱 브라우저 감지 결과에 따라 외부 브라우저 안내 UI를 렌더링하는 클라이언트 컴포넌트
"use client";
...
export default function OpenInBrowserClient() {
  ...
  const [appName, setAppName] = useState("앱 내 브라우저");
  ...
  function handleOpenExternal() {
    if (android) {
      window.location.href = `intent://${window.location.host}${redirect}#Intent;scheme=https;package=com.android.chrome;S.browser_fallback_url=${encodeURIComponent(targetUrl)};end`;
    } else {
      handleCopy();
    }
  }
