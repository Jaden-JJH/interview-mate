...
export default function CareerDescriptionPage() {
  ...
  const canSubmit =
    position.trim().length > 0 && keyExperience.trim().length >= 30;
    ...
    <div className="flex flex-1 flex-col px-5 pt-6 pb-32">
      ...
      {step === "form" && (
        <div className="flex flex-col gap-4">
          ...
          <div>
            <label className="text-[13px] font-semibold text-[var(--gray-900)] mb-1.5 block">
              핵심 경력 및 성과 <span className="text-[var(--blue-primary)]">*</span>
            </label>
            <textarea
              rows={8}
              value={keyExperience}
              onChange={(e) => setKeyExperience(e.target.value)}
              placeholder={"프로젝트별로 자유롭게 적어주세요.\n\n예:\n- 사내 대시보드 개발 (React, 2023)\n  → MAU 2만→8만 성장, 로딩 속도 40% 개선\n- 결제 시스템 리팩터링\n  → 장애율 90% 감소"}
